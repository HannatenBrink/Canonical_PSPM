...

{
    ...
    while (1)
    {
        ...
        /*k1[0] =  SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0);
         k1[1] =  SelectionGradient2(BASE_PNTDIM, point, Equation, BIFPARTWO, 0);
         
         //h = (1/pow(10,ceil(log(max(k1[0],k1[1]))/log(10))))*0.0001;
         if (max(k1[0],k1[1]) >1)
         {
         h=0.001;
         }
         else if (max(k1[0],k1[1]) <0.0001)
         {
         h=1;
         }
         else
         ...
         ///Selectiongradient voor trait 1/////////
         memset((void *)tanvec, 0, pntdim*sizeof(double));  //geen idee of dit steeds moet worden gereset maar voor de zekerheid toch maar doen
         tanvec[0] = 1.0;
         COPY(pntdim, point, 1, oldpointselec, 1); //copieert het punt in oldpointselect
         point[0] = k1[0]*(h/2)+point[0];             //bepaald nieuwe punt aan de hand van de k1
         fp = FindPoint(pntdim, point, tanvec, DYTOL, RHSTOL, MAXITER, Equation);  //Bepaald de nieuwe oplossing in dit punt
         if (fp != SUCCES)
         {
         NumProcError(__FILE__, __LINE__, retval);
         ErrorMsg(__FILE__, __LINE__, "Failed to locate a solution point during calculation for k2[0], change h");
         break;
         }
         k2[0] = SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0);   //Bepaald de selectiegradient in dit nieuwe punt
         COPY(pntdim, oldpointselec, 1, point, 1); //copieert het punt in oldpointselect naar point zodat dat weer goed staat
         point[0] = k2[0]*(h/2)+point[0];   //bepaald nieuwe punt aan de hand van de k2
         memset((void *)tanvec, 0, pntdim*sizeof(double));  //geen idee of dit steeds moet worden gereset maar voor de zekerheid toch maar doen
         tanvec[0] = 1.0;
         fp = FindPoint(pntdim, point, tanvec, DYTOL, RHSTOL, MAXITER, Equation);  //Bepaald de nieuwe oplossing in dit punt
         if (fp != SUCCES)
         {
         NumProcError(__FILE__, __LINE__, retval);
         ErrorMsg(__FILE__, __LINE__, "Failed to locate a solution point during calculation for k3[0], change h");
         break;
         }
         k3[0] = SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0);   //Bepaald de selectiegradient in dit nieuwe punt
         COPY(pntdim, oldpointselec, 1, point, 1); //copieert het punt in oldpointselect naar point zodat dat weer goed staat
         point[0] = k3[0]*h+point[0];   //bepaald nieuwe punt aan de hand van de k2
         memset((void *)tanvec, 0, pntdim*sizeof(double));  //geen idee of dit steeds moet worden gereset maar voor de zekerheid toch maar doen
         tanvec[0] = 1.0;
         fp = FindPoint(pntdim, point, tanvec, DYTOL, RHSTOL, MAXITER, Equation);  //Bepaald de nieuwe oplossing in dit punt
         if (fp != SUCCES)
         {
         NumProcError(__FILE__, __LINE__, retval);
         ErrorMsg(__FILE__, __LINE__, "Failed to locate a solution point during calculation for k4[0], change h");
         break;
         }
         k4[0] = SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0); //Bepaald de selectiegradient in dit nieuwe punt
         COPY(pntdim, oldpointselec, 1, point, 1); //copieert het punt in oldpointselect naar point zodat dat weer goed staat
         ReportMsg(1, "%10.5E\t\n",point[0]+(h/6)*(k1[0]+2*k2[0]+2*k3[0]+k4[0]));
         
         ///Selectiongradient voor andere traits/////////
         
         memset((void *)tanvec, 0, pntdim*sizeof(double));  //geen idee of dit steeds moet worden gereset maar voor de zekerheid toch maar doen
         tanvec[0] = 1.0;
         COPY(pntdim, point, 1, oldpointselec, 1); //copieert het punt in oldpointselect
         oldpar = parameter[BIFPARTWO];
         parameter[BIFPARTWO] = oldpar+(h/2)*k1[1];    //bepaald nieuwe punt aan de hand van de k1
         fp = FindPoint(pntdim, point, tanvec, DYTOL, RHSTOL, MAXITER, Equation);  //Bepaald de nieuwe oplossing in dit punt
         if (fp != SUCCES)
         {
         NumProcError(__FILE__, __LINE__, retval);
         ErrorMsg(__FILE__, __LINE__, "Failed to locate a solution point during calculation for k2[1], change h");
         break;
         }
         k2[1] = SelectionGradient2(BASE_PNTDIM, point, Equation, BIFPARTWO, 0);   //Bepaald de selectiegradient in dit nieuwe punt
         COPY(pntdim, oldpointselec, 1, point, 1); //copieert het punt in oldpointselect naar point zodat dat weer goed staat
         parameter[BIFPARTWO] = k2[1]*(h/2)+oldpar;   //bepaald nieuwe punt aan de hand van de k2
         memset((void *)tanvec, 0, pntdim*sizeof(double));  //geen idee of dit steeds moet worden gereset maar voor de zekerheid toch maar doen
         tanvec[0] = 1.0;
         fp = FindPoint(pntdim, point, tanvec, DYTOL, RHSTOL, MAXITER, Equation);  //Bepaald de nieuwe oplossing in dit punt
         if (fp != SUCCES)
         {
         NumProcError(__FILE__, __LINE__, retval);
         ErrorMsg(__FILE__, __LINE__, "Failed to locate a solution point during calculation for k3[1], change h");
         break;
         }
         k3[1] = SelectionGradient2(BASE_PNTDIM, point, Equation, BIFPARTWO, 0);   //Bepaald de selectiegradient in dit nieuwe punt
         COPY(pntdim, oldpointselec, 1, point, 1); //copieert het punt in oldpointselect naar point zodat dat weer goed staat
         parameter[BIFPARTWO] = k3[1]*h+oldpar;   //bepaald nieuwe punt aan de hand van de k2
         memset((void *)tanvec, 0, pntdim*sizeof(double));  //geen idee of dit steeds moet worden gereset maar voor de zekerheid toch maar doen
         tanvec[0] = 1.0;
         fp = FindPoint(pntdim, point, tanvec, DYTOL, RHSTOL, MAXITER, Equation);  //Bepaald de nieuwe oplossing in dit punt
         if (fp != SUCCES)
         ...
         k4[1] = SelectionGradient2(BASE_PNTDIM, point, Equation, BIFPARTWO, 0); //Bepaald de selectiegradient in dit nieuwe punt
         COPY(pntdim, oldpointselec, 1, point, 1); //copieert het punt in oldpointselect naar point zodat dat weer goed staat
         
         
         
         point[0] = point[0]+(h/6)*(k1[0]+2*k2[0]+2*k3[0]+k4[0]);
         parameter[BIFPARTWO] = oldpar+(h/6)*(k1[1]+2*k2[1]+2*k3[1]+k4[1]);
