...

{
    ...
    while (1)
    {
        ...
        else
        {
...
#if (!ALLOWNEGATIVE)
                    CurveEnd = CurveEnd || (point[i]*pnt_scale[i] < DYTOL);
#endif
                }
            }
            ...
            point[0] = SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0)*0.001+point[0];
            
            parameter[12] = SelectionGradient2(BASE_PNTDIM, point, Equation, 12, 0)*0.001+parameter[12];
...
//    }
  ...
  while (1)
      ...
      if (setpoint)
	{
	  ...
	  Stepreduce = 1;
	  COPY(pntdim, oldpoint, 1, point, 1);  //copieert oldpoint naar point
	...
	{
	  retval = FindPoint(pntdim, point, tanvec, DYTOL, RHSTOL, MAXITER, Equation); //zoekt punt (en veranderd punt (maar ook y?)
	  ...
	  if (retval == SUCCES)
	      ...
	      if (Report2Terminal)
		{
		  ...
		  for (i=0; i<BASE_PNTDIM; i++)
		    ReportMsg(1, "%10.5E\t", point[i]*pnt_scale[i]);
            ReportMsg(1, "%10.5E\t", SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0));
            ReportMsg(1, "%10.5E\t", SelectionGradient2(BASE_PNTDIM, point, Equation, 12, 0));
		  ReportMsg(1, "\n");
		}
	      else
		{
      ...
      else if ((retval = SetScales(point, BASE_PNTDIM, pntdim)))
	{
...
#endif
#if (CANONICAL)
        CurveEnd = CurveEnd || ((point[0]*pnt_scale[0]) < minbound) || ((point[0]*pnt_scale[0]) > maxbound) || ((SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0) <CANTOL) && SelectionGradient2(BASE_PNTDIM, point, Equation, 12, 0) < CANTOL);
#endif
	    }
	  ...
	  COPY(pntdim, point, 1, oldpoint, 1); //copieert point naar oldpoint
	  Stepchange = 1;
...
#if CANONICAL
        COPY(pntdim, tanvec, 1, selvec, 1);
        //parstep = SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0)*0.001;
        selvec[0] = SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0);
        AXPY(pntdim, 0.001, selvec, 1, point, 1);
        parameter[12] = SelectionGradient2(BASE_PNTDIM, point, Equation, 12, 0)*0.001+parameter[12];
#else
