...

{
    ...
    //double evopar2 = parameter[((int)(parameter[15]+0.5))];
    //int evopar3 = parameter[((int)(parameter[16]+0.5))];
    ...
    while (1)
    {
        ...
        // Otherwise generate output and predict new point on the curve
            ...
            if (Report2Terminal)
            {
                ...
                ReportMsg(1, "%10.5E\t", parameter[BIFPARTWO]);
                ReportMsg(1, "%10.5E\t\n", SelectionGradient2(BASE_PNTDIM, point, Equation, BIFPARTWO, 0));
                
            }
        
        for (i=0; (pntnr > 20) && (i<pntdim); i++)
        {
            CurveEnd = CurveEnd || ((point[0]*pnt_scale[0]) < minbound) || ((point[0]*pnt_scale[0]) > maxbound) || ((SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0) <CANTOL) && (SelectionGradient2(BASE_PNTDIM, point, Equation, BIFPARTWO, 0) <CANTOL));
#if (!ALLOWNEGATIVE)
        ...
        double selec1 =  SelectionGradient(BASE_PNTDIM, point, Equation, 0, 0);
        double selec2 =SelectionGradient2(BASE_PNTDIM, point, Equation, BIFPARTWO, 0);
        
       point[0] = selec1*DELTAS+point[0];
       parameter[BIFPARTWO] = selec2*DELTAS+parameter[BIFPARTWO];
